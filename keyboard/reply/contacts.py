...
def hotel_suggestions(user) -> list[dict[str, str]]:
    querystring = {"destinationId": user.destinationId, "pageNumber": "1", "pageSize": user.quantity_count,
...
def hotel_suggestions_highest(user) -> list[dict[str, str]]:
    querystring = {"destinationId": user.destinationId, "pageNumber": "1", "pageSize": user.quantity_count,
                   "checkIn": '2020-01-08', "checkOut": '2020-01-15',
...
# 0528626382
def best_seller(user: Optional[Users]) -> list[dict[str, str]]:
    querystring = {"destinationId": user.destinationId, "pageNumber": "1", "pageSize": user.quantity_count,
                   "checkIn": '2020-01-08', "checkOut": '2020-01-15',
                   "adults1": "1", "priceMin": user.min_price, "priceMax": user.max_price, "sortOrder": 'DISTANCE_FROM_LANDMARK',
                   "currency": "USD", "locale": "en_US", "landmarkIds": 'City center'}
    s = requests.Session()
    response = s.get(url='https://hotels4.p.rapidapi.com/properties/list', headers=headers, params=querystring)
    pattern = r'(?<=,)"results":.+?(?=,"pagination)'
    find = re.search(pattern, response.text)
    if find:
        ...
        for i_key in suggestions['results']:  # Обрабатываем результат
            ...
            try:
                ...
                if float(user.distance) > float(i_key["landmarks"][0]["distance"].split()[0].replace(',', '.')):
...
def city_markup(user) -> Optional[InlineKeyboardMarkup]:
    cities = city_founding(
        user)  # Функция "city_founding" уже возвращает список словарей с нужным именем и id
...
def photos(id_hotel: str, user=None):
    """
    ...
    if user.hotels_count is None:
        ...
        return res[0:int(user.hotels_count)]
...
#print(best_seller(user))
#print(photos(id_hotel='220590', user=user))
